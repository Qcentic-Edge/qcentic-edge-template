# Plugins: Configurable resources and pages

Source: https://filamentphp.com/docs/5.x/plugins/configurable-resources-and-pages.md — fetched 2026-08-21
...
> Note: While this guide is in the plugins section, configurable resources and pages work in any `PanelProvider` - you don't need to be building a plugin to use them. They are especially useful for plugins because they let plugin authors expose flexible configuration to their users.
...
The configuration class follows the same fluent API patterns as the rest of Filament - setter methods return `$this` for chaining, and getter methods retrieve stored values.

> Tip: The `ResourceConfiguration` base class already includes a `slug()` method for overriding the URL slug. You only need to add properties specific to your plugin.
...
> Tip: If you're building a plugin class, you would register configurations inside the `register(Panel $panel)` method instead.
